 ...
 */
public enum DocumentCaptureViewImageFilter: Int {
...
public class DocumentCaptureView: UIView, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    /// Sets the imageFilter based on `DocumentCaptureViewImageFilter` Enum.
    public var imageFilter: DocumentCaptureViewImageFilter = .Normal {
        didSet {
    ...
    public func captureImage(completion: (image: UIImage, feature: CIRectangleFeature?) -> Void) {
        ...
        self.stillImageOutput.captureStillImageAsynchronouslyFromConnection(videoConnection!, completionHandler: { (imageSampleBuffer, error) -> Void in
            ...
            if self.borderDetectionEnabled && self.detectionConfidenceValid() {
                if let rectangleFeature = self.biggestRectangle(DocumentCaptureView.highAccuracyRectangleDetector.featuresInImage(enhancedImage) as! [CIRectangleFeature]) {
                    enhancedImage = self.perspectiveCorrectedImage(enhancedImage, feature: rectangleFeature)
                }
            }
            
            UIGraphicsBeginImageContext(CGSize(width: enhancedImage.extent.size.width, height: enhancedImage.extent.size.height))

            UIImage(CIImage: enhancedImage).drawInRect(CGRect(x: 0, y: 0, width: enhancedImage.extent.size.width, height: enhancedImage.extent.size.height))
            
            let image = UIGraphicsGetImageFromCurrentImageContext()
            UIGraphicsEndImageContext()
            
            completion(image: image, feature: self.biggestRectangle(DocumentCaptureView.highAccuracyRectangleDetector.featuresInImage(enhancedImage) as! [CIRectangleFeature]))
        })
    ...
    public func captureOutput(captureOutput: AVCaptureOutput!, didOutputSampleBuffer sampleBuffer: CMSampleBuffer!, fromConnection connection: AVCaptureConnection!) {
        ...
        if self.borderDetectionEnabled {
            if self.borderDetectFrame {
                ...
                self.borderDetectLastRectangleFeature = self.biggestRectangle(DocumentCaptureView.highAccuracyRectangleDetector.featuresInImage(image) as! [CIRectangleFeature])
