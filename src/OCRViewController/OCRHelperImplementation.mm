...
inline cv::Mat cropMRZArea(cv::Mat input) {
    cv::Mat cropped = input.clone();
    cv::Mat output = input.clone();
    cv::Size size = input.size();
    cv::Mat rectKernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(13, 5));
    cv::Mat sqKernel = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(21, 21));
    cv::Mat rectKernel2 = cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3));
    ...
    gradX = cv::Mat(255 * ((gradX - minVal) / (maxVal - minVal)));
    ...
    cv::findContours(thresh.clone(), contours, cv::RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE);
    ...
    for(size_t i = 0; i < contours.size(); i++) {
        ...
        if(ar > 5 && crWidth > 0.75) {
            ...
            NSLog(@"before cut");
            cv::Mat(output, roi).copyTo(cropped);
...
{
    // convert to Mat
    cv::Mat origMat = [image CVMat];
    cv::Mat inputMat = origMat.clone();
    cv::Mat croppedMrz = cropMRZArea(inputMat);
    cv::Mat gray;
    cv::cvtColor(croppedMrz, gray, cv::COLOR_BGR2GRAY);
    // prepare for ocr
...
+ (UIImage *) prepareOCR:(UIImage*) image;
{
    cv::Mat origMat = [image CVMat];
    cv::Mat gray;
    cv::cvtColor(origMat, gray, cv::COLOR_BGR2GRAY);
    // prepare for ocr
    gray.convertTo(gray, CV_8UC1);
    cv::threshold(gray, gray, 127, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
    UIImage* result =  [UIImage imageWithCVMat:gray];
...
//    
//    
//    cv::Mat output(output5);
    UIImage* result =  [UIImage imageWithCVMat:SWTImage];
    return result;
}
